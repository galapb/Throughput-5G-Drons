...
def generar_df(seed=None, n_usuarios=None):
    ...
    valores = np.random.uniform(10, 2048, size=n_usuarios)  # 10 MB y 2048 MB.
    valores = np.round(valores, 2)  

    dron_lat = 40.327 + np.random.rand() * 0.01
    dron_lon = -3.765 + np.random.rand() * 0.01
    ...
    return df, dron_lat, dron_lon
tiempo_tx_maximo = 0.0
suma_nuevos_maximos=0
max_iteracion=0
#Archivo calculadora 5G
print("Ejercicio: Posicion del dron ALEATORIA.")
print("Número de Iteraciones: 30.")
print("Número de Usuarios: 10-20.")
v_dron = 15 #km/h
...
for i in range(15):
    ...
    df, dron_lat, dron_lon = generar_df(seed=i, n_usuarios=None)
