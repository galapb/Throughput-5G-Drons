...
max_iteracion=0
print("Ejercicio: Posicion del dron punto Intermedio centroídes.")
...
for i in range(15):
    ...
    ###########################################################################################################################
    #Calculamos distancias de los usuarios al dron
    ###########################################################################################################################
    ...
    #Distancia de los usuarios al dron
    df['distancia_a_dron (m)'] = df.apply(lambda row: distancia(row['latitud'], row['longitud'], dron_lat, dron_lon), axis=1)
    ...
    distancias = df['distancia_a_dron (m)']
    DD_us="Uplink"
...
media_tiempo_tx_result = round(media_tiempo_tx_final/60, 2)
